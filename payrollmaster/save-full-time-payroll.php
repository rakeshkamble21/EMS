...
if($_SERVER['REQUEST_METHOD']=='POST')
{
    ...
    for ($i = 0; $i < count($name_array); $i++) {
       ...
       //foreach($att_array  as $key=>$i ) { 
        ...
        $stmt = $db->prepare("INSERT INTO full_time_salary(emp_id,emp_name,salary_perday,absent_days,work_hour,advance_deduction,clean_salary,nett_salary,date) VALUES(?,?,?,?,?,?,?,?,?)"); //Fetching all the records with input credentials
