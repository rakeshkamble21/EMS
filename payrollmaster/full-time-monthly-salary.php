...
<div class="col-md-10">
<h3 style="text-align:center;margin-top:20px">12 hours Employee Monthly Payroll</h3>
...
<form action=" " id="form1" method="post">
            ...
            <a href="process-full-time-payroll.php"><input type="button" class="btn btn-danger" name="home" value="back" /></a>&nbsp;&nbsp;
...
<table id="tblsalary" class="table stripped">
  <thead>
   ...
   -->
                ...
                if($_SERVER['REQUEST_METHOD'] == 'POST')
                {
                    ...
                    try
                    {
                     ...
                     // present and absent query  $sql="select employee.salary, attendance.emp_name,attendance.emp_id ,count(case when attendance.attendance ='Absent' then 1 end) as absent_count ,count(case when attendance.attendance ='Present' then 1 end) as present_count ,count(distinct date) as Tot_count from attendance JOIN employee ON employee.emp_id=attendance.emp_id where attendance.date between ? and ? group by attendance.emp_id"; 
                        $sql="select SUM(full_time_salary.clean_salary) AS tot,SUM(full_time_salary.nett_salary) AS total ,full_time_attendance.work_hours,full_time_attendance.emp_name,full_time_attendance.emp_id,count(case when full_time_attendance.attendance ='Absent' then 1 end) as absent_count,SEC_TO_TIME(SUM(TIME_TO_SEC(work_hours))) AS TotalTime from full_time_attendance JOIN full_time_salary ON full_time_salary.emp_id=full_time_attendance.emp_id where full_time_attendance.date between ? and ? group by full_time_salary.emp_id";
                        $stmt = $db->prepare($sql);
      ...
      <input type="button" onclick="submitForm('save-full-time-monthly-payroll.php?from=<?php echo $from_date?>&to=<?php echo $to_date?>')" name="update" class="btn btn-success" value="Save">
     <a href="full-time-salary-print.php"> <input type="button" class="btn btn-inof" value="next"></a>
  </form>
