...
if($_SERVER['REQUEST_METHOD']=='POST')
{
    ...
    for ($i = 0; $i < count($name_array); $i++) {
       ...
       //foreach($att_array  as $key=>$i ) { 
        ...
        $stmt = $db->prepare("update salary set nett_salary=? where emp_id=? and date=?"); //Fetching all the records with input credentials
        $stmt->bind_param("iis",$salary_array[$i],$empid_array [$i],$currentDateTime); //Where s indicates string type. You can use i-integer, d-double
        $stmt->execute();
    ...
    if ($result>0) {
        ...
        echo '</script>';
        header('refresh:2; url="process-payroll.php');
      }
      else
      {
