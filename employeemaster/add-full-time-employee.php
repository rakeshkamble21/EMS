                 ...
                 try
{
 ...
 $sql = "select max(emp_id) as max from full_time_emp";
 $stmt = $db->prepare($sql); 
 $stmt->execute();
 ...
 <div class="row">
     ...
     <div class="col-md-8">
         <form action="" method="post">
             <div class="note">
                 <p>Register a 12 hours working Employee.</p>
             </div>
             ...
             <div class="form-content">
                 ...
                 <a href="../home.php"><button class="btn btn-danger" type="button">Home</button></a>
         ...
         echo '</script>';
             ...
             else  if(!preg_match("/^(\+\d{1,3}[- ]?)?\d{10}$/", $mobile))
             {
              ...
              else
              {
                 ...
                 $stmt = $db->prepare("INSERT INTO full_time_emp(emp_id,firstname,lastname,mobile,address,salary,gender) VALUES(?,?,?,?,?,?,?)"); //Fetching all the records with input credentials
                 $stmt->bind_param("issisis",$inc,$first,$last,$mobile,$address,$salary,$gender); //Where s indicates string type. You can use i-integer, d-double
                  $stmt->execute();
                  ...
                  $name=$first.' '.$last;
                  $stmt1 = $db->prepare("INSERT INTO full_time_emp_advance(emp_id,emp_name,advance,deposit_advance,remain_advance,reg_date) VALUES(?,?,?,?,?,?)"); //Fetching all the records with input credentials
                  $stmt1->bind_param("isiiis",$inc,$name,$advance,$advance_taken,$remain_advance,$currentDateTime); //Where s indicates string type. You can use i-integer, d-double
                  $stmt1->execute();
                 ...
                 //  $stmt2->bind_param("isiis",$inc,$name,$advance_taken,$remain_advance,$currentDateTime); //Where s indicates string type. You can use i-integer, d-double
                 //  $stmt2->execute();
