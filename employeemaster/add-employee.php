                    ...
                    try
{
    ...
    $sql = "select max(emp_id) as max from employee";
    $stmt = $db->prepare($sql); 
    $stmt->execute();
                ...
                else  if(!preg_match("/^(\+\d{1,3}[- ]?)?\d{10}$/", $mobile))
                {
                 ...
                 else
                 {
                    //echo $first."".$last."".$mobile."".$address."".$salary."".$gender;
                     $stmt = $db->prepare("INSERT INTO employee(emp_id,firstname,lastname,mobile,address,salary,gender) VALUES(?,?,?,?,?,?,?)"); //Fetching all the records with input credentials
                     ...
                     $name=$first.' '.$last;
                     $stmt1 = $db->prepare("INSERT INTO advance(emp_id,emp_name,advance,deposit_advance,remain_advance,reg_date) VALUES(?,?,?,?,?,?)"); //Fetching all the records with input credentials
