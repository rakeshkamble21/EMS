...
<div class="col-md-10">
<h3 style="text-align:center;margin-top:20px">Employee Work details Report</h3>
...
<form action=" " id="form1" method="post">
            <div class="col-md-6">
                            ...
                            <div class="form-group">
                                            <select class="form-control"  name="work_des" >
                                                <option value="select" >Select work</option>
                                                <!-- <option value="Project Manager">packaging</option> -->
                                                <option value="datecutting">Date cutting</option>
                                                <option value="datefiltering">Date filtering</option>
                                            </select>
                              </div>         
   ...
   -->
        <?php
                ...
                if($_SERVER['REQUEST_METHOD'] == 'POST')
                {
                    ...
                    $work_description=$_POST['work_des'];
                    if ( $fromdate) {
                    ...
                    try
                    {
                     ...
                     // present and absent query  $sql="select employee.salary, attendance.emp_name,attendance.emp_id ,count(case when attendance.attendance ='Absent' then 1 end) as absent_count ,count(case when attendance.attendance ='Present' then 1 end) as present_count ,count(distinct date) as Tot_count from attendance JOIN employee ON employee.emp_id=attendance.emp_id where attendance.date between ? and ? group by attendance.emp_id"; 
                        $sql="select sum(work_assign_master.work_done) as totalwork, work_assign_master.date,work_assign_master.work_description,work_assign_master.emp_id, work_assign_master.emp_name from work_assign_master JOIN employee ON work_assign_master.emp_id=employee.emp_id where date between ? and ? and work_description=? GROUP BY work_assign_master.emp_id";
                        $stmt = $db->prepare($sql);
                        $stmt->bind_param("sss",$from_date,$to_date,$work_description);
                        $stmt->execute();
