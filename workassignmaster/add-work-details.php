...
<div class="col-md-10">
<h3 style="text-align:center;margin-top:20px">Employee daily work entry</h3>
...
<table id="tblsalary" class="table stripped">
  <thead>
   ...
   -->
                ...
                if($_SERVER['REQUEST_METHOD'] == 'POST')
                {
                    ...
                    try
                    {
                     ...
                     // present and absent query  $sql="select employee.salary, attendance.emp_name,attendance.emp_id ,count(case when attendance.attendance ='Absent' then 1 end) as absent_count ,count(case when attendance.attendance ='Present' then 1 end) as present_count ,count(distinct date) as Tot_count from attendance JOIN employee ON employee.emp_id=attendance.emp_id where attendance.date between ? and ? group by attendance.emp_id"; 
                        $sql="select * from work_assign where date=?";
                        $stmt = $db->prepare($sql);
                        ...
                        <td><input type="text" readonly value="<?php echo $row['work_description'] ?>" name="work_description[]" class="form-control"></td>
                        <td><input type="text"  value="0" name="work_done[]" class="form-control"> </td>
                        <td><input type="text"  readonly value="<?php echo $row['date']?>" name="dates[]" class="form-control"> </td>
      ...
      <input type="button" onclick="submitForm('save-work-assign-details.php')" name="update" class="btn btn-success" value="Save">
