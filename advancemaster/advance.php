                                ...
                                <?php
                                          ...
                                          try
                                          {
                                              ...
                                              while ($row = $result->fetch_assoc()) 
                                              {
                                                  ...
                                                  //echo $row4;
                                                  echo "<option value='".$row1.' '.$row5."'>".$row1.' '.'  '.$row4."</option>";
...
<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST')
    {
        ...
        $result1=mysqli_query($db,"select * from advance where emp_id='$empid'");
                ...
                else  if(preg_match("/[^0-9]/", $advance))
                {
                 ...
                 else
                 {
                   ...
                   /// echo $empid."<br>".$name."<br>".$advance."<br>".$deposit_taken."<br>".$newadvance."<br>".$currentDateTime;
                     $stmt = $db->prepare("update advance set advance=?,remain_advance=?,advance_date_taken=? where emp_id=?"); //Fetching all the records with input credentials
                     $stmt->bind_param("iisi",$advance,$newadvance,$currentDateTime,$empid); //Where s indicates string type. You can use i-integer, d-double
                     $stmt->execute();
                     ...
                     $stmt2->bind_param("isiiis", $empid,$name,$newadvance,$deposit_taken,$newadvance,$currentDateTime); //Where s indicates string type. You can use i-integer, d-double
                     $stmt2->execute();
