...
<div class="col-md-10">
<h3 style="text-align:center;margin-top:20px">Update 12 hours Emloyee Attendance</h3>
...
<table class="table table-bordered" style="margin-top:20px">
        ...
        <?php
           ...
           // echo $pasdate;
                try
                    {
                        $sql = "SELECT * FROM full_time_attendance where emp_id=? and date=?";
                        $stmt = $db->prepare($sql); 
...
if($_SERVER['REQUEST_METHOD']=='POST')
{
    ...
    // for ($i = 0; $i < count($name_array); $i++) {
       ...
       //foreach($att_array  as $key=>$i ) { 
        ...
        // echo  " ".$workhour_array;
        // echo "".$att_array;
         $attendance = $db->prepare("update full_time_attendance set emp_name=?,attendance=?,in_time=?,out_time=?,work_hours=? where emp_id=? and date=?");
